<template>
  <div>
    <div class="alert alert-warning text-center" role="alert">
      ...
      <div v-for="k in kategori" v-bind:key="k.idCategory" style="display: inline !important;">
        <a class="link-kategori" style="margin-left: 20px; color: #ac6730;" @click="filterKategori(k.strCategory)"> {{k.strCategory}} </a>
      </div>
    </div>
  ...
  <div class="row mt-1 justify-content-center">
        <div class="col-md-8">
            <h1 class="text-center">Mau cari makanan apa?</h1>
            <div class="input-group mb-1 mt-1">
                <input type="text" class="form-control" v-model="cari" placeholder="Ketik makanan yang ingin kamu cari!" style="text-align:center;">
            </div>
        </div>
    </div>
    <hr>
      ...
      <div class="col-md-4" v-for="item in filterMenu" v-bind:key="item.idMeal">
        <div class="card mb-3">
            ...
            <div class="card-body text-center">
              ...
              <button type="button" class="btn btn-outline-danger btn-block"  data-toggle="modal" data-target="#staticBackdrop" @click="detail(item.idMeal)">Detail Makanan</button>
              <div class="addFav mt-3">
                <nuxt-link to="/favorite" style="margin-top:10px;"><button type="button" class="btn btn-warning btn-block" v-if="sLogin" @click="addFav(item.idMeal)">Tambah Ke Favorite</button></nuxt-link>
              </div>
            </div>
        </div>
      </div>
...
<style scoped>
  ...
  .link-kategori:hover{
    border-bottom: solid 2px #341d08;
  }
  .link-area:hover{
